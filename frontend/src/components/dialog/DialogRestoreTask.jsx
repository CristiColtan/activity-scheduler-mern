...
const DialogRestoreTask = ({ open, setOpen, trashedTasks, setTrashedTasks, taskData }) => {
  const { currentUser } = useSelector((state) => state.user);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  console.log("task restore:", taskData);
  ...
  const restoreTask = async (taskID) => {
    try {
      ...
      const res = await fetch(`http://localhost:8081/backend/task/restore-task/${taskID}`, {
        method: "PUT",
        headers: {
      ...
      <MyModal open={open} setOpen={setOpen}>
        <div className='py-4 w-full flex flex-col gap-4 items-center justify-center'>
          ...
          {taskData && <p className='text-center text-black mt-1 font-serif md:text-lg text-base'>
            Are you sure you want to restore <span className='font-bold'>{taskData.title}</span> ?
          </p>}
          ...
          <div className='py-3 flex justify-between gap-4 bg-white w-auto'>
            ...
            hover:bg-blue-400' onClick={() => restoreTask(taskData._id)}>
