...
const AdminTaskTable = ({ tasks, setTasks }) => {
  ...
  const handleDeleteAssetOnClick = async (taskID, asseturlIndex) => {
    try {
      const res = await fetch("http://localhost:8081/backend/admin/delete/asset", {
        method: "PUT",
        headers: {
      ...
      setTasks((prevTasks) => prevTasks.map((task) => (task._id === data._id ? { ...task, asseturls: task.asseturls.filter((_, index) => index !== asseturlIndex) } : task)));
    } catch (error) {
  ...
  const handleDeleteActivityOnClick = async (taskID, activityIndex) => {
    try {
      const res = await fetch("http://localhost:8081/backend/admin/delete/activity", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setTasks((prevTasks) => prevTasks.map((task) => (task._id === data._id ? { ...task, activities: task.activities.filter((_, index) => index !== activityIndex) } : task)));
    } catch (error) {
      console.error(error.message);
      return;
    }
        ...
        <tr className={clsx('text-black hover:bg-gray-200', !expand && 'border-b border-black')}>
          <td className='py-2'>
            <div className='flex items-center gap-2'>
              ...
              <p className='font-serif'>{task?.title}
                <span className='font-semibold'>{(task?.is_trashed === "Yes") && " (Trashed)"}</span>
              </p>
            </div>
          </td>
          ...
          <td className='p-2'>
            <div className='flex justify-end gap-3'>
              <button className='hidden lg:block ml-1' onClick={() => navigate(`/task/${task._id}`)}>
                <FaRegFolderOpen
                          ...
                          task.asseturls.map((url, index) => (
                            <div key={index} className='w-full flex items-center  gap-10 mb-5'>
                              ...
                              <button className='px-6 py-2 rounded 
                                ...
                                font-medium'
                                onClick={() => handleDeleteAssetOnClick(task._id, index)}>
                                Delete
                              </button>
                            </div>))
                        }
                          ...
                          task.activities.map((item, index) => (
                            <div key={index} className='w-full grid grid-cols-2'>
                              ...
                              <div className='flex items-center'>
                                ...
                                font-medium -translate-y-3'
                                  onClick={() => handleOpenEditOnClick(item, task._id, index)}>
  ...
  return (
    <>
      ...
      <AdminDialogEditActivity open={openEdit}
        setOpen={setOpenEdit}
        allTasks={tasks}
