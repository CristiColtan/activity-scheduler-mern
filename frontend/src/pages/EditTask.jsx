...
const EditTask = () => {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:8081/backend/task/get-task-edit/${params.id}`, {
          credentials: "include",
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8081/backend/task/update/${params.id}`, {
        method: "PUT",
        headers: {
          ...
          <div className='w-full md:w-1/2 flex flex-col justify-center md:justify-start'>
            ...
            <div className='flex gap-4 mt-5'>
              <div className='w-full'>
                ...
                <input type="date" required id='date' onChange={handleChange} value={formData.date ? formData.date.split('T')[0] : ''}
                  className='border py-1 px-2 placeholder-black focus:ring-2 ring-blue-300 outline-none
