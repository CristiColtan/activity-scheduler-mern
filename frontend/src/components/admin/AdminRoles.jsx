...
const AdminRoles = () => {
    ...
    const deleteRole = async (rolee) => {
        try {
            ...
            const res = await fetch("http://localhost:8081/backend/admin/delete/role", {
                method: "DELETE",
                credentials: "include",
                headers: {
    ...
    const fetchRoles = async () => {
        try {
            ...
            const res = await fetch("http://localhost:8081/backend/admin/get/roles", {
                credentials: "include",
            });
    ...
    return (
        <>
                    ...
                    <>
                        {error ? (<p className='text-red-500 text-4xl'>{error}</p>) : (
                            <div className='w-full py-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 xl:gap-10 px-6'>
                                    ...
                                    roles && roles.length > 0 ? (
                                        <>
                                            {roles.map((role, index) => (
                                                <div key={index} className='h-fit border border-gray-300 bg-white shadow-lg rounded p-4 gap-8'>
                                                    ...
                                                    <div className='flex flex-col items-center justify-center text-center'>
                                                        ...
                                                        <span className='font-serif font-semibold mt-2 whitespace-normal overflow-hidden
                                                        line-clamp-1 lg:overflow-hidden lg:whitespace-pre lg:text-ellipsis
                                                        hover:overflow-visible hover:line-clamp-none'>{role}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </>) : (
                                ...
                                <button className='h-fit border border-gray-300 hover:bg-gray-100 bg-white 
                                shadow-lg rounded p-4 gap-6'
                                    onClick={() => setOpenDialogAddRole(true)}>
                                    <div className='flex items-center justify-center py-3'>
                                        <div className='text-center'>
                                            <FaPlus className='text-5xl lg:text-7xl mb-2' />
                                            <span className='text-center font-serif font-thin mt-2'>Costum</span>
                                        </div>
                                    </div>
                                </button>
                            </div>
            ...
            <AdminDialogAddRole open={openDialogAddRole}
