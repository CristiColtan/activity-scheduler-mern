...
const DialogDuplicateTask = ({ open, setOpen, tasks, setTasks, taskData }) => {
  ...
  const handleDuplicateTask = async (taskID) => {
    try {
      ...
      const res = await fetch(`http://localhost:8081/backend/task/duplicate-task/${taskID}`, {
        method: 'POST',
        headers: {
      ...
      <MyModal open={open} setOpen={setOpen}>
        <div className='py-4 w-full flex flex-col gap-4 items-center justify-center'>
          ...
          <div className='py-3 flex justify-between gap-4 bg-white w-auto'>
            ...
            hover:bg-blue-400' onClick={() => handleDuplicateTask(taskData._id)}>
