...
const icons = {
  alert: (
    <HiBellAlert className="text-black md:text-5xl text-3xl"></HiBellAlert>
  ),
  message: (
    <BiSolidMessage className="text-black md:text-5xl text-3xl"></BiSolidMessage>
  ),
};
  ...
  const markAsRead = async (NotifID) => {
    try {
      ...
      setNotifications((prevNotifications) =>
        prevNotifications.filter((n) => n._id !== NotifID)
      );
      handleCancel();
    } catch (error) {
        ...
        <div className="py-4 w-full flex flex-col gap-4 items-center justify-center">
          ...
          <div className="py-3 flex justify-between gap-4 bg-white w-auto">
            ...
            <button
              className="px-6 py-2 rounded font-sans border-2 border-gray-400 bg-white ml-5
            hover:bg-gray-300 font-semibold"
              onClick={() => markAsRead(notifData._id)}
            >
              Mark as read
            </button>
          </div>
