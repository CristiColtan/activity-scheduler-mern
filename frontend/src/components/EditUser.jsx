    ...
    const handleEditUser = async (userID) => {
        try {
            ...
            if (currentUser.is_team_manager === "Yes") {
                res = await fetch(`http://localhost:8081/backend/team-manager/edit/team-member/${userID}`,
                    {
                        method: "PUT",
                        headers: {
            ...
            if (currentUser.is_admin === "Yes") {
                res = await fetch(`http://localhost:8081/backend/admin/edit/team-manager/${userID}`,
                    {
                        method: "PUT",
                        headers: {
                            "Content-Type": "application/json",
