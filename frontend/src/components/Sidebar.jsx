...
const Sidebar = () => {
  ...
  const NavLink = ({ el }) => {
    return (
      <Link
        ...
        className={clsx(
          "w-full lg:w-3/4 flex gap-2 px-3 py-2 rounded-full items-center text-black text-base hover:bg-[#2564ed2d]",
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 px-5 py-1">
      <p
        className="flex flex-col gap-0 md:gap-4 lg:text-4xl text-3xl
                        duration-500 transform lg:px-8
                        py-2 font-bold text-gray-800 font-serif
      ...
      <div className="">
        <button
          className={clsx(
            "w-full lg:w-3/4 flex gap-2 p-2 items-center text-lg text-red rounded-full",
            {
              hidden: currentUser.is_admin !== "Yes",
              "bg-red-600 text-white hover:bg-red-600":
                location.pathname === "/settings",
              "hover:bg-red-200": location.pathname !== "/settings",
            }
          )}
          onClick={() => {
            navigate("/settings");
