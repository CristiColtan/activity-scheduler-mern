...
import { priority_styles, task_type, bgs, bgs_transparent } from '../utils/tableImports.js';
              ...
              <div className='w-full md:w-1/2 space-y-6'>
                <div className='flex items-center gap-5'>
                  ...
                  <div className='flex items-center gap-1.5'>
                    ...
                    <span className='text-black font-serif uppercase'>{task.stage}</span>
                  </div>
                </div>

                <p className='font-thin'>Created at: {new Date(task.date).toDateString()}</p>

                <div className='flex items-center gap-8 px-4 py-2 border-y border-gray-500'>
                  <div className='space-x-2'>
                    <span className='font-serif'>Assets:{" "} <span>{" "}{task.assets.length}</span></span>
                    <span className='text-gray-500 pl-2 pr-2'>|</span>
                    <span className='font-serif'>Sub-Tasks:{" "} <span>{" "}{task.subTasks.length}</span></span>
                  </div>
                </div>
                      ...
                      task.team.map((m, index) => (
                        <div key={index} className='flex gap-4 py-2 items-center border-t border-gray-500'>
                          <div className='w-10 h-10 rounded-full text-base -mr-1 bg-blue-600 flex items-center justify-center text-white'>
                            <span className='text-center'>{getInitials2(m.name)}</span>
                          </div>
...
const Activities = ({ activity }) => {
  ...
  return (
    <>
      <div className='w-full flex gap-10 2xl:gap-20 flex-col md:flex-row overflow-y-auto justify-between'>
