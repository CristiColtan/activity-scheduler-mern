...
const DialogAssignHours = ({ open, setOpen, taskId, userId }) => {
    ...
    const fetchHours = async () => {
        try {
            ...
            const res = await fetch(`http://localhost:8081/backend/normal-user/get-hours/${taskId}`, {
                credentials: "include",
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch('http://localhost:8081/backend/normal-user/assign-hours', {
                method: "PUT",
                headers: {
            ...
            <MyModal open={open} setOpen={setOpen}>
                ...
                <div className='mt-2 flex flex-col gap-10'>
                    <div className='w-full flex flex-col gap-1'>
                        <label htmlFor='hours' className='font-normal text-base'>Logged hours: 
                            <span className='font-thin ml-2 mr-5'>{hours || "Nothing assigned today."}</span>
                            Total logged hours: <span className='font-thin ml-1'>{totalHours || "0"}</span></label>
                        <input type="number" id="hours" onChange={handleChange} value={formData.hours}
